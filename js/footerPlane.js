...
// Date: July 31,2019
// Version: 2.5
// Functionality: Adds a flying plane to the footer
...
$(document).ready(function () {
    ...
    animatePlane(planeObj, 0, 0); // Start animation
})

// Defines arrays that specifies plane height and angle of rotation in certain sections of flight path. Based on S-shaped curve
...
// Changes plane type and speed on hover
function hoverPlane(planeObj) {
    var plane = planeObj.node; //HTML node
...
function animatePlane(planeObj, percent, prevHeight) {
    ...
    // Recall function after set interval (based on speed)
    setTimeout(function () {
        animatePlane(planeObj, percent, height)
    }, parseInt(xDelta / planeObj.speed * 10000));
}